...
def _answer_general_question(message: str) -> str:
    ...
    if not msg:
        return (
            "I'm RideWise assistant. I can answer general questions about this app "
            "and predict bike demand hourly or daily. Try asking about features or models."
        )
    if "help" in msg or ("how" in msg and ("use" in msg or "work" in msg)):
        return (
            "RideWise forecasts bike demand.\n"
            "- Hourly: ask 'bikes tomorrow at 17:00?'\n"
            "- Daily: ask 'total bikes on 2025-11-25?'\n"
            "You can also ask about features, models, or weather options."
        )
    if "feature" in msg or "capabilit" in msg:
        return (
            "Features: hourly and daily predictions, weather integration "
            "(temperature, humidity, wind, condition), weekend/holiday awareness, "
            "dashboard history and charts, and a simple chatbot."
        )
    if "model" in msg or "algorithm" in msg:
        return (
            "The app uses pre-trained models for hourly and daily demand. "
            "Models are loaded from local files (e.g., ridewise_hourly_gb_model.pkl). "
            "Predictions use calendar and weather features."
        )
    if "weather" in msg or "condition" in msg:
        return (
            "Supported weather options: "
            + ", ".join(WEATHER_OPTIONS)
            + ". Weather affects demand in predictions."
        )
    if "hourly" in msg and "daily" in msg:
        return (
            "Hourly prediction estimates demand for a specific hour; "
            "daily prediction estimates the total demand for the day with an hourly breakdown."
        )
    if "login" in msg or "sign in" in msg or "signup" in msg or "sign up" in msg:
        return (
            "Use Sign Up to create an account, then Sign In. "
            "You must be signed in to access the dashboard and chatbot."
        )
    if "history" in msg or "chart" in msg or "dashboard" in msg:
        return (
            "Your last 20 predictions appear on the dashboard with charts: "
            "overall trend, type split (hourly vs daily), plus separate hourly/daily history."
        )
    if "data" in msg and ("save" in msg or "store" in msg or "privacy" in msg):
        return (
            "Prediction requests are stored locally per user in 'prediction_history.json' "
            "to power the dashboard. No external transmission by default."
        )
    return (
        "I can help with general app questions and predictions. "
        "Ask about features, models, weather, history, or say 'help' for guidance."
    )
...
def api_chat():
    ...
    if not has_intent:
        # If Gemini available, answer generally; else use built-in knowledge base.
        if gemini_model is not None:
            try:
                resp = gemini_model.generate_content(
                    "You are RideWise assistant. Briefly answer about the app or its capabilities in 1-3 short sentences. Question: " + message
                )
                ...
                pass
        return jsonify({"reply": _answer_general_question(message)})
